...
def criar_cabecalho(janela):
    ...
    tk.Label(marca, text="UNIVERSIDADE DE MARÍLIA", font=FONTE_MARCA_SECUNDARIA, bg=COR_AZUL, fg=COR_CARTAO).pack(anchor="w", pady=(1, 0))
    ...
    tk.Label(sistema, text="CENTRAL DE CHAMADOS", font=("Arial", 11, "bold"), bg=COR_AZUL, fg=COR_CARTAO).pack(anchor="e", pady=(4, 0))


def criar_rodape(janela):
    rodape = tk.Frame(janela, bg=COR_CARTAO, height=48, highlightbackground=COR_BORDA, highlightthickness=1)
    rodape.pack(side="bottom", fill="x")
    rodape.pack_propagate(False)
    tk.Label(rodape, text="UNIMAR  •  UNIVERSIDADE DE MARÍLIA", font=("Arial", 8, "bold"), bg=COR_CARTAO, fg=COR_TEXTO_SECUNDARIO).pack(side="left", padx=34, pady=15)
    tk.Label(rodape, text="CENTRAL DE ATENDIMENTO  •  AMBIENTE ACADÊMICO", font=FONTE_MICRO, bg=COR_CARTAO, fg=COR_TEXTO_SECUNDARIO).pack(side="right", padx=34, pady=15)
...
def criar_titulo(parent, texto, subtitulo=None):
    ...
    tk.Label(textos, text=texto, font=FONTE_TITULO, bg=COR_FUNDO, fg=COR_AZUL_MUITO_ESCURO).pack(anchor="w")
    if subtitulo:
        tk.Label(textos, text=subtitulo, font=FONTE_SUBTITULO, bg=COR_FUNDO, fg=COR_TEXTO_SECUNDARIO).pack(anchor="w", pady=(4, 0))


def criar_card(parent, padx=30, pady=25):
    sombra = tk.Frame(parent, bg=COR_SOMBRA)
    sombra.pack(fill="x", padx=52, pady=(0, 2))
    card = tk.Frame(sombra, bg=COR_CARTAO, highlightbackground=COR_BORDA, highlightthickness=1, padx=padx, pady=pady)
...
def criar_campo(parent, texto, show=None):
    ...
    tk.Label(bloco, text=texto.upper(), font=FONTE_LABEL, bg=COR_CARTAO, fg=COR_TEXTO).pack(anchor="w", pady=(0, 5))
...
def criar_botao(parent, texto, comando, secundario=False):
    ...
    botao.pack(fill="x", pady=(7 if secundario else 4, 0))
    return botao


def criar_mensagem(parent):
    mensagem = tk.Label(parent, text="", font=FONTE_PEQUENA, bg=COR_CARTAO, wraplength=470, justify="center")
...
def criar_aviso(parent, texto, tipo="info"):
    ...
    tk.Label(aviso, text=texto, font=FONTE_PEQUENA, bg=fundo, fg=cor, wraplength=500, justify="center").pack()
    return aviso
...
def criar_painel_lateral(parent):
    ...
    tk.Label(corpo, text="UNIMAR", font=("Arial", 30, "bold"), bg=COR_AZUL_MUITO_ESCURO, fg=COR_CARTAO).pack(anchor="w")
    tk.Label(corpo, text="UNIVERSIDADE DE MARÍLIA", font=("Arial", 8, "bold"), bg=COR_AZUL_MUITO_ESCURO, fg="#D7EBF7").pack(anchor="w", pady=(2, 28))
    ...
    tk.Label(corpo, text="Central de\nChamados", font=("Arial", 22, "bold"), bg=COR_AZUL_MUITO_ESCURO, fg=COR_CARTAO, justify="left").pack(anchor="w", pady=(7, 16))
    tk.Label(corpo, text="Atendimento e suporte\npara a comunidade acadêmica.", font=("Arial", 10), bg=COR_AZUL_MUITO_ESCURO, fg="#D7EBF7", justify="left").pack(anchor="w")
    bloco = tk.Frame(corpo, bg="#0A4D77", padx=15, pady=15)
    bloco.pack(side="bottom", fill="x")
    tk.Label(bloco, text="UNIVERSIDADE • EDUCAÇÃO • TECNOLOGIA", font=FONTE_MICRO, bg="#0A4D77", fg=COR_CARTAO, justify="left").pack(anchor="w")
    return painel
...
def abrir_login(janela):
    ...
    tk.Label(coluna, text="ACESSO INSTITUCIONAL", font=("Arial", 9, "bold"), bg=COR_FUNDO, fg=COR_AZUL).pack(anchor="w", pady=(2, 5))
    tk.Label(coluna, text="Bem-vindo à UNIMAR", font=("Arial", 23, "bold"), bg=COR_FUNDO, fg=COR_AZUL_MUITO_ESCURO).pack(anchor="w")
    tk.Label(coluna, text="Entre para acessar o ambiente de atendimento.", font=FONTE_SUBTITULO, bg=COR_FUNDO, fg=COR_TEXTO_SECUNDARIO).pack(anchor="w", pady=(5, 18))
    card = tk.Frame(coluna, bg=COR_CARTAO, highlightbackground=COR_BORDA, highlightthickness=1, padx=28, pady=25)
    card.pack(fill="x")
    criar_badge(card, "Acesso seguro ao ambiente")
    tk.Label(card, text="Acesse sua conta", font=FONTE_SECAO, bg=COR_CARTAO, fg=COR_TEXTO).pack(anchor="w", pady=(0, 20))
...
def abrir_cadastro(janela):
    ...
    coluna = tk.Frame(area, bg=COR_FUNDO)
    coluna.pack(side="left", fill="both", expand=True)
    tk.Label(coluna, text="COMUNIDADE UNIMAR", font=("Arial", 9, "bold"), bg=COR_FUNDO, fg=COR_AZUL).pack(anchor="w", pady=(0, 5))
    tk.Label(coluna, text="Criar cadastro", font=("Arial", 23, "bold"), bg=COR_FUNDO, fg=COR_AZUL_MUITO_ESCURO).pack(anchor="w")
    tk.Label(coluna, text="Cadastre-se para acessar a Central de Chamados da UNIMAR.", font=FONTE_SUBTITULO, bg=COR_FUNDO, fg=COR_TEXTO_SECUNDARIO).pack(anchor="w", pady=(5, 15))
    card = tk.Frame(coluna, bg=COR_CARTAO, highlightbackground=COR_BORDA, highlightthickness=1, padx=28, pady=20)
    card.pack(fill="x")
...
def abrir_inicio(janela, usuario):
    ...
    tk.Label(area, text="Bem-vindo ao ambiente institucional de atendimento da UNIMAR.", font=FONTE_SUBTITULO, bg=COR_FUNDO, fg=COR_TEXTO_SECUNDARIO).pack(anchor="w", pady=(0, 20))
    topo = tk.Frame(area, bg=COR_FUNDO)
    topo.pack(fill="x")
    card_usuario = tk.Frame(topo, bg=COR_CARTAO, highlightbackground=COR_BORDA, highlightthickness=1, padx=24, pady=20)
    ...
    tk.Label(card_usuario, text=usuario.get("nome", "usuário"), font=("Arial", 18, "bold"), bg=COR_CARTAO, fg=COR_TEXTO).pack(anchor="w", pady=(5, 1))
    tk.Label(card_usuario, text=usuario.get("email", ""), font=FONTE_PEQUENA, bg=COR_CARTAO, fg=COR_TEXTO_SECUNDARIO).pack(anchor="w")
    card_institucional = tk.Frame(topo, bg=COR_AZUL_MUITO_ESCURO, padx=24, pady=20)
    ...
    tk.Label(card_institucional, text="Atendimento institucional", font=("Arial", 16, "bold"), bg=COR_AZUL_MUITO_ESCURO, fg=COR_CARTAO).pack(anchor="w", pady=(5, 2))
    tk.Label(card_institucional, text="Um ambiente preparado para apoiar\na comunidade acadêmica.", font=FONTE_PEQUENA, bg=COR_AZUL_MUITO_ESCURO, fg="#D7EBF7", justify="left").pack(anchor="w")
    painel = tk.Frame(area, bg=COR_CARTAO, highlightbackground=COR_BORDA, highlightthickness=1, padx=26, pady=22)
    ...
    tk.Label(painel, text="A área de chamados está em desenvolvimento. Em breve este ambiente poderá receber as funcionalidades de atendimento do sistema.", font=FONTE_PADRAO, bg=COR_CARTAO, fg=COR_TEXTO_SECUNDARIO, wraplength=760, justify="left").pack(anchor="w")
    ...
    sair = tk.Button(area, text="SAIR DO SISTEMA", font=FONTE_BOTAO, bg=COR_CARTAO, fg=COR_AZUL, activebackground=COR_AZUL_CLARO, activeforeground=COR_AZUL_MUITO_ESCURO, relief="solid", bd=1, highlightthickness=0, cursor="hand2", command=lambda: abrir_login(janela), padx=20, pady=9)
    sair.pack(anchor="e")
    criar_rodape(janela)
